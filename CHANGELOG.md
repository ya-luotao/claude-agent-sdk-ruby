...
### Fixed
- **`exit`, `Interrupt` and signal exceptions raised inside hooks, `can_use_tool`, and SDK MCP resource readers / prompt generators no longer escape dispatch.** Each is now an ordinary callback failure, reported as the exception class and message (`"SystemExit: exit"`): exactly one response is written for the request (for hooks and `can_use_tool` the error control response any other exception produces; for `resources/read` / `prompts/get` a JSON-RPC `-32603` error), and the session keeps serving later requests. Previously no response was written and the reactor stopped; in the default `:thread` scheduling Ruby also re-raised the worker thread's `SystemExit` on the main thread, which answered with a misleading `Cancelled` error and then ended the process. The conversion runs inside the callback's thread hop at every dispatch site, including both `HookMatcher#timeout` variants. Cancellation (`Async::Stop`, hook timeouts) still propagates. A `callback_wrapper` observes a `RuntimeError` whose `#cause` is the original exception. Observers and message blocks are unchanged. (#119)

### Changed
- **`exit` / `Interrupt` / signal exceptions from SDK MCP tool handlers are now reported with the exception class** (`"SystemExit: exit"`, `"Interrupt: Interrupt"`, `"SignalException: SIGTERM"`) in the `isError` text, matching the hook / `can_use_tool` / resource / prompt paths (see Fixed). Since #77 the text was the bare message, so a handler calling `exit` produced just `"exit"`. (#119)
