...
- `ClaudeAgentSDK.offload { }` — public escape hatch for `:inline` hosts: runs a CPU-bound or scheduler-opaque piece of a callback on a plain thread so it cannot stall the shared reactor. No-op outside a reactor.
