...
- **Disk and store paths agree on sidechain classification and blank summaries**: a corrupt, blank or non-object first transcript line no longer makes one path list a session the other hides (the disk reader now classifies from the first parseable entry, as the store path did), and whitespace-only summaries/titles are blank on both. (#67, #75)
