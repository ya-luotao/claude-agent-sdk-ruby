...
- Attributes are the names declared through `attr_accessor` / `attr_reader` / `attr_writer` on a `Type` subclass (Type overrides them to record the names, inherited) plus `Type.declare_attributes` for a hand-written reader (`RateLimitEvent#data`); the rules live in `types/attributes.rb`. `#[]`, `#[]=` and camelCase `method_missing` reach attributes (and `attr?` predicates) plus any method user code defined (owner outside the `ClaudeAgentSDK::` namespace and below `Type` in the ancestors, or a singleton method); positive registry hits are cached per class keyed by the caller's spelling. A name resolving to any other public method (`to_h`, `freeze`) is treated as undefined (`#[]` nil, `#[]=` no-op, camelCase `NoMethodError`, `respond_to?` false; 0.37 warned, 1.0 enforces). Types the user builds and passes IN (option values, `HookMatcher`, hook outputs, `PermissionResult*`, `PermissionUpdate`, `PermissionRuleValue`) declare `strict_attributes`: an unknown key on `.new` / `#[]=` raises `ArgumentError` naming the class, the key and the known attributes. CLI-parsed types stay lenient, and so does anything built through `.from_hash` / `.wrap` (a fiber-local lenient scope that also covers nested values) — SDK code hydrating CLI data into a strict type must use those, never `.new`. `spec/unit/strict_attributes_spec.rb` pins the strict classification; `spec/unit/type_attribute_access_spec.rb` pins that every public reader/writer is a declared attribute
