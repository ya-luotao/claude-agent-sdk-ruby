...
`CLIInstaller` (`lib/claude_agent_sdk/cli_installer.rb`) downloads a pinned CLI into `vendor/claude` (relative to `CLIInstaller.root` when set — the Railtie sets it to `Rails.root` — else to `Dir.pwd`, resolved at call time; `root` is a plain frozen-String ivar, absolutized on set, read lock-free) for Docker/CI deploys: dist-tag → concrete version, manifest SHA-256 + declared size, streamed download (bounded by that size) to a random-named sibling temp file (`O_EXCL`), verify, `chmod 0755`, atomically write `VERSION` = version + verified checksum, and **finally** rename the temp file into place. Stdlib only; every failure (filesystem errors included) is wrapped in `CLIInstallError` with `cause` preserved.
...
- **`TranscriptMirrorBatcher`** buffers `transcript_mirror` stdout frames and flushes to `#append` per-turn (`batched`, default) or near-realtime (`eager`); StandardError failures retry with backoff but never raise — they surface as `MirrorErrorMessage` (process-exit exceptions — `exit`, `Interrupt`, signals — from a store adapter propagate)
- Retried batches may overlap prior writes, so adapters should dedupe by `entry["uuid"]` (with a fiber-native `:inline` adapter, a cooperatively cancelled append may also be half-applied — same dedupe covers it)
...
`async` installs a Fiber scheduler, but most Ruby libraries (pg, mysql2, ActiveRecord pools) key state on `Thread.current` and are thread-safe, not fiber-safe. `FiberBoundary.invoke` (`lib/claude_agent_sdk/fiber_boundary.rb`) hops every user-supplied callback (tool handlers, hooks, permission callbacks, message blocks, observers) to a plain thread before invoking it. Control-request callbacks (hooks, `can_use_tool`, SDK MCP tool/resource/prompt handlers) go through `FiberBoundary.invoke_callback`: a `SystemExit` / `SignalException` raised by the callback is carried out of the hop, `Query#handle_control_request` writes the normal error response, then re-raises the original — the CLI always gets its answer and process termination is never swallowed (#119). Consequence: the thread hop severs `break`/`return`/`next` from the surrounding method — SDK loops yielding user callbacks must keep loop control outside the invoked block (see `Client#receive_response`; user `break` is bridged via `.invoke_iteration`).

Opt-in `ClaudeAgentOptions#callback_scheduling: :inline` (issue #47) skips the hop and runs callbacks in place on the reactor fiber — for hosts that are fiber-isolated end to end (solid_queue fiber workers with `IsolatedExecutionState.isolation_level = :fiber`). The mode is plumbed per-call (Query kwarg; the SDK-MCP dispatch path carries it via fiber storage — `Fiber[FiberBoundary::DISPATCH_KEY]` — so a server shared by concurrent sessions is never mutated), never via a thread-local. Timeout-bounded store-adapter calls hop by default regardless of session mode — unless the ADAPTER declares itself fiber-native via an optional `callback_scheduling` method returning `:inline` (probed by `SessionStores.store_callback_scheduling` at batcher construction / resume-materialization entry; issue #47 phase 3), in which case they run in place under a cooperative timeout that still surfaces as `JoinTimeout`. Inline hook timeouts are cooperative (`with_timeout`). Public `ClaudeAgentSDK.offload { }` hops one heavy block manually. `ClaudeAgentOptions#callback_wrapper` (issue #47 phase 2) is optional middleware composed around every user-callback dispatch BEFORE the hop, so it runs on the callback's own execution context (worker thread in `:thread`, reactor fiber in `:inline`) — in Rails, `ClaudeAgentSDK::Railtie.callback_wrapper` (see Rails Integration; a bare `executor.wrap` lambda deadlocks under dev code reloading); it travels the SDK-MCP dispatch path inside the same fiber-storage scope as the mode, and also composes around timeout-bounded store-adapter dispatch (batcher appends, resume-materialization calls) inside the bound. `SdkMcpServer#callback_scheduling=` / `#callback_wrapper=` validate like their `ClaudeAgentOptions` counterparts (String coerced, invalid values raise).

### Rails Integration

`lib/claude_agent_sdk.rb` requires `railtie.rb` only `if defined?(Rails::Railtie)` — nothing Rails-related loads otherwise (`spec/unit/require_surface_spec.rb`). `ClaudeAgentSDK::Railtie` sets `CLIInstaller.root ||= Rails.root` in an initializer ordered `before: :load_config_initializers` (so vendored-CLI discovery ignores the process cwd, and app initializers can still override), loads the `claude_agent_sdk:install_cli` rake task (`lib/claude_agent_sdk/tasks/claude_agent_sdk.rake`, also loadable from a plain Rakefile via `require 'claude_agent_sdk/tasks'`; guarded against double definition; it does not boot the app, so it anchors to `Rails.root` itself unless `CLIInstaller.root` is already set) and hosts `Railtie.callback_wrapper`: executor-wrapped callbacks, except with a locking executor (reloading enabled / `allow_concurrency == false`, mirroring railties' `configure_executor_for_concurrency`), where a callback thread entering the executor deadlocks against its waiting caller — there it runs unwrapped and releases AR connections; already-active executor (`:inline`) → straight through. The generator lives at `lib/generators/claude_agent_sdk/install/` (explicit `namespace`: Thor would derive `claude_agent_s_d_k`). Rails specs live in `spec/rails/`, excluded from the default run by the root `.rspec`; run them with `BUNDLE_GEMFILE=gemfiles/rails_8.gemfile bundle exec rspec --options spec/rails/.rspec` (`rails_7_1.gemfile` for the floor).
...
- Type definitions live in `lib/claude_agent_sdk/types/`, one file per area: `base` (`Type`, `Type::OptionValue`), `attributes` (attribute declarations and access rules, loaded by `base`), `content_blocks`, `messages` (incl. `SystemMessage` subclasses, rate-limit types), `hooks` (`HOOK_EVENTS`, inputs, outputs), `permissions`, `mcp`, `option_values` (value types `ClaudeAgentOptions` holds), `sessions`, `options` (`ClaudeAgentOptions`). Shared constants sit with the types that use them. `types.rb` only `require_relative`s the parts; add new types to the matching part file (each part requires `base` itself)
- Attributes are the names declared through `attr_accessor` / `attr_reader` / `attr_writer` on a `Type` subclass (Type overrides them to record the names, inherited) plus `Type.declare_attributes` for a hand-written reader (`RateLimitEvent#data`); the rules live in `types/attributes.rb`. `#[]`, `#[]=` and camelCase `method_missing` reach attributes (and `attr?` predicates) plus any method user code defined (owner outside the `ClaudeAgentSDK::` namespace and below `Type` in the ancestors, or a singleton method); positive registry hits are cached per class keyed by the caller's spelling. A name resolving to any other public method (`to_h`, `freeze`) warns once per (class, name) and still works through 0.x. Types the user builds and passes IN (option values, `HookMatcher`, hook outputs, `PermissionResult*`, `PermissionUpdate`, `PermissionRuleValue`) declare `strict_attributes`: an unknown key on `.new` / `#[]=` warns once per (class, key). Both warnings go through `Deprecation.warn_once_at_caller`, and both flip to 1.0 behaviour (raise / attributes only) with the single `Type::ENFORCE_ATTRIBUTES` constant. CLI-parsed types stay lenient, and so does anything built through `.from_hash` / `.wrap` (a fiber-local lenient scope that also covers nested values) — SDK code hydrating CLI data into a strict type must use those, never `.new`. `spec/unit/strict_attributes_spec.rb` pins the strict classification; `spec/unit/type_attribute_access_spec.rb` pins that every public reader/writer is a declared attribute
