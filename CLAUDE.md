...
- Types the user builds and passes IN (option values, `HookMatcher`, hook outputs, `PermissionResult*`, `PermissionUpdate`, `PermissionRuleValue`) declare `strict_attributes`: an unknown key on `.new` / `#[]=` warns once per (class, key) via `Deprecation.warn_once_at_caller` (raises in 1.0 — flip `Type::UNKNOWN_ATTRIBUTE_ACTION`). Setters and public readers (`type`, `hook_event_name`) are known. CLI-parsed types stay lenient, and so does anything built through `.from_hash` / `.wrap` (a fiber-local lenient scope that also covers nested values) — SDK code hydrating CLI data into a strict type must use those, never `.new`. `spec/unit/strict_attributes_spec.rb` pins the classification
