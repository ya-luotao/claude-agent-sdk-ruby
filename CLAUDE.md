...
`SubprocessCLITransport#find_cli` probes, in order: `CLAUDE_CLI_PATH` → `CLIInstaller.installed_path` (project-local vendored binary, deliberately ahead of `PATH` so a pinned install is hermetic) → `which claude` → common install locations.
...
The whole check → download → record → publish sequence runs under an exclusive `flock` on `<dir>/.install.lock`, so concurrent installs into one directory serialize. The idempotency shortcut re-hashes the binary against the recorded checksum and makes **no** network call (offline re-boot must work, given a pinned version); a mismatch or a legacy single-line `VERSION` triggers a clean reinstall.
