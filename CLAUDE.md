...
`lib/claude_agent_sdk.rb` requires `railtie.rb` only `if defined?(Rails::Railtie)` — nothing Rails-related loads otherwise (`spec/unit/require_surface_spec.rb`). `ClaudeAgentSDK::Railtie` only loads the `claude_agent_sdk:install_cli` rake task (`lib/claude_agent_sdk/tasks/claude_agent_sdk.rake`, also loadable from a plain Rakefile via `require 'claude_agent_sdk/tasks'`; guarded against double definition) and hosts `Railtie.callback_wrapper`: executor-wrapped callbacks, except with a locking executor (reloading enabled / `allow_concurrency == false`, mirroring railties' `configure_executor_for_concurrency`), where a callback thread entering the executor deadlocks against its waiting caller — there it runs unwrapped and releases AR connections; already-active executor (`:inline`) → straight through. The generator lives at `lib/generators/claude_agent_sdk/install/` (explicit `namespace`: Thor would derive `claude_agent_s_d_k`). Rails specs live in `spec/rails/`, excluded from the default run by the root `.rspec`; run them with `BUNDLE_GEMFILE=gemfiles/rails_8.gemfile bundle exec rspec --options spec/rails/.rspec` (`rails_7_1.gemfile` for the floor).
