...
The SDK also carries the active OTel context (including baggage) into the reactor fibers created by `query()` and standalone `Client.open`, and across callback worker-thread hops. `OTelObserver` session spans therefore keep the surrounding caller span as their parent in both default `:thread` and opt-in `:inline` mode. For `Client`, keep the intended parent active while receiving messages. Context is captured per operation, not when an observer is constructed, so sequential observer reuse does not retain an earlier caller's parent. This uses OTel's scoped context API only when loaded; it does not copy application thread-local state or change callback scheduling.
