...
Fix:
- Install Claude Code CLI (Node.js required), or vendor a pinned binary with `ClaudeAgentSDK::CLIInstaller.install_pinned` (0.34.0+; installs the CLI version the gem release was tested against, `CLIInstaller::PINNED_CLI_VERSION`) or `CLIInstaller.install(version: 'stable')` (or a concrete version) for a pin of your own. Downloads into `vendor/claude/`; discovery prefers it over `PATH` — since 0.30.0.
